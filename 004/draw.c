...
void GameOver(void){
    ...
    while ((ch = getch()) != 'q') {
        wclear(snakeys_world);
        mvprintw(offsety+WORLD_HEIGHT/2-5,offsetx+WORLD_WIDTH/2-5, "Game Over");
        mvprintw(offsety+WORLD_HEIGHT/2-4,offsetx+WORLD_WIDTH/2-8, "Your Score is %d",Score);
        mvprintw(offsety+WORLD_HEIGHT/2-1,offsetx+i, "Please enter 'q' to quit");
        mvprintw(WORLD_HEIGHT-3,offsetx+5, "Copyright © 人工与AI ALL right reserverd");
        box(snakeys_world, 0 , 0);
        wrefresh(snakeys_world);
        if(offsetx+i<WORLD_WIDTH-12&&n%2==0){
            i++;
